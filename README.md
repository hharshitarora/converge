...
Ask an agent to onboard a new customer and it will do five or six things across four
...
times and it creates **10 duplicate objects**.
...
$ converge run "Onboard Acme Corp, Pro tier, primary contact ops@acme.com"
  ...
  Slack   + acme-corp-onboarding                  create
  ...
  Plan: 6 to create, 0 to update, 0 unchanged

  CONVERGED  in 2 passes, 36ms
```

Run it again and it does nothing, because there is nothing left to do:

```
  Plan: 0 to create, 0 to update, 6 unchanged
  CONVERGED  in 1 pass, 2ms
```
...
| Duplicates after running the same job 3× | **0** | **10** |
